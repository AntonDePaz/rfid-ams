...
class student extends Database{
	...
	public function insert_student($data = []){
		//if(isset($_POST['data'])){
			...
			$rfid = $data[0];
			$idnumber = $data[1];
			$fn = ucwords(strtolower($data[2]));
			$mn = ucwords(strtolower($data[3]));
			$ln = ucwords(strtolower($data[4]));
			$section = $data[5];
			$year = $data[6];
			$course =  ucwords(strtolower($data[7]));
            $major =  ucwords(strtolower($data[8]));
            $sem = $_SESSION['sem_id'];
                ...
                if($result){
					...
					if($result->num_rows > 0){
						while($row = $result->fetch_assoc()){
                            ...
                            $value .= "<td><a class='btn btn-default btn-xs edit' title='edit' data-id=".$row['student_id']."><i class='fa fa-edit'> edit</i></a> &nbsp;&nbsp; <a class='btn btn-default btn-xs del' title='delete' data-id=".$row['student_id']."> <i class='fa fa-trash-o'> delete</i></a> </td>";
                          $value .= '</tr>';
						}
						return $value;
				   }
		...
		if(isset($_SESSION['sy_id'])){
			...
			$rfid = $data[1];
			$idnumber = $data[2];
			$fn = ucwords(strtolower($data[3]));
			$mn = ucwords(strtolower($data[4]));
			$ln = ucwords(strtolower($data[5]));
			$section = $data[6];
			$year = $data[7];
			$course = ucwords(strtolower($data[8]));
			$major = ucwords(strtolower($data[9]));
			$drfid = $data[10];
			$didnumber = $data[11];
			$sem = $_SESSION['sem_id'];
            $semester = 'sem'.trim($sem);
